...
module.exports = {
  ...
  flattenMessageData({ data, type }, preferedDepth = 0) {
    function isNumericString(n) {
      return !isNaN(parseFloat(n)) && isFinite(n);
    }

    function continueFlatten(object, currentLevel = 1){
      if(object == null)
        return false
    
      if(currentLevel > preferedDepth)
        return true
      ...
      for (var key in object) {
        ...
        if (typeof object[key] == 'object') {
          if(continueFlatten(object[key], ++currentLevel))
            return true
        }
      }
      return false;
        ...
        case 'object':
          {
            ...
            // Keep last object when using InfluxDB 
            if(preferedDepth > 0 && !continueFlatten(value)){
              target[keyPrefix] = value;
              break;
            }
